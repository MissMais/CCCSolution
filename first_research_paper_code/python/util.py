...
class LSTM_Model():
    ...
    def model_LSTM(self):
        ...
        self.hist = self.
        (self.p_train, epochs = self.epochs, steps_per_epoch = self.steps_per_epoch,
...
#         self.arch_plot()
    ...
    def plot_bot_decision(self):
        ...
        try:
            ...
            ax.plot(bot_ideal, label='Ideal case (%.2f)' % bot_ideal[-1])
            ax.plot(bot_pred, label='From prediction (%.2f)' % bot_pred[-1])
            ax.plot(bot_pred_update, label='From prediction (updated) (%.2f)' % bot_pred_update[-1])
            ax.plot(ideal / ideal[0] * 100.0, label='Stock value(%s)' % self.ts)
            ax.set_xlabel("Days")
            ax.set_ylabel("Percentage growth")
            ax.legend()

            # Save the figure
            plt.savefig('../images/Bot_prediction_%d_%d_%d_%d_%s_%s.png' % (
                self.depth, int(self.naive), self.past_history, self.forward_look, self.ts, int(self.custom_loss)))
            ...
            print(f"Error in plot_bot_decision: {e}")
